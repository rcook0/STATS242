...
def chapter08(*, out_dir: str, events: str, seasonality: str = "hour_of_day") -> tuple[str, str]:
    """Durations + ACD(1,1) baseline with seasonality adjustment and residual diagnostics."""
    _prov(out_dir, "chapter08", params={"events": events, "seasonality": seasonality}, inputs={"events": events})
    df = read_csv_ts(events)
    ts = df["ts"].sort_values()
    dt = ts.diff().dt.total_seconds().dropna()
    dt = dt[dt > 0]

    # seasonality adjustment: divide by average duration in each bin
    if seasonality == "hour_of_day":
        hours = ts.iloc[1:].dt.hour
        by_bin = pd.DataFrame({"dt": dt.to_numpy(), "bin": hours.to_numpy()}).groupby("bin")["dt"].mean()
        dt_adj = dt / hours.map(by_bin).to_numpy()
        bin_desc = "hour of day"
    elif seasonality == "minute_of_day":
        mod = (ts.iloc[1:].dt.hour * 60 + ts.iloc[1:].dt.minute)
        by_bin = pd.DataFrame({"dt": dt.to_numpy(), "bin": mod.to_numpy()}).groupby("bin")["dt"].mean()
        dt_adj = dt / mod.map(by_bin).to_numpy()
        bin_desc = "minute of day"
    else:
        dt_adj = dt
        bin_desc = "none"

    x = pd.Series(dt_adj).replace([np.inf, -np.inf], np.nan).dropna().to_numpy()
    fit = models.acd11_fit(x)

    resid = None
    psi = np.asarray(fit.get("psi_hat", []), dtype=float)
    if psi.size == x.size and np.all(np.isfinite(psi)) and np.all(psi > 0):
        resid = pd.Series(x / psi, name="std_resid").replace([np.inf, -np.inf], np.nan).dropna()
    ...
    figs = [
        ...
        plots.plot_hist(pd.Series(dt_adj), "Durations (seasonality-adjusted)", os.path.join(fig_dir, "dur_hist_adj.png"), bins=80),
    ]
    if resid is not None and len(resid) > 20:
        acf_res = diag.acf_simple(resid - resid.mean(), max_lag=40)
        figs += [
            plots.plot_hist(resid, "ACD standardized residuals (x/psi)", os.path.join(fig_dir, "acd_resid_hist.png"), bins=80),
            plots.plot_qq(resid, "QQ: standardized residuals vs Exp(1) (heuristic)", os.path.join(fig_dir, "acd_resid_qq.png")),
            plots.plot_acf_bar(acf_res, "ACF: standardized residuals", os.path.join(fig_dir, "acd_resid_acf.png")),
        ]

    summary = {
        "n_events": int(len(ts)),
        "n_durations": int(len(dt)),
        "seasonality": seasonality,
        "seasonality_bin": bin_desc,
        "fit": fit,
        "resid": {
            "n": int(len(resid)) if resid is not None else 0,
            "mean": float(resid.mean()) if resid is not None and len(resid) else None,
            "var": float(resid.var()) if resid is not None and len(resid) else None,
        },
    }
    _write_json(out_dir, "artifacts/summary.json", summary)
    ...
    mc = model_card("Chapter 08", {
        ...
        "Model": "ACD(1,1), Exp innovations (baseline), MLE",
        "Diagnostics": "standardized residuals and ACF (heuristic)",
    }, notes=[
        "The toy dataset is synthetic; use this runner to validate plumbing and diagnostics.",
    ])
    ...
    return write_report(out_dir, "Chapter 08 — Durations & ACD baseline (research-grade)", sections)
...
def chapter09(
    *,
    ...
    grid: str = "1min",
    horizon: str = "1min",
    grids: str = "2s,5s,10s,30s,1min,5min",
) -> tuple[str, str]:
    """Spread measures + microstructure noise signature plot (research-grade baseline)."""
    _prov(out_dir, "chapter09", params={"quotes": quotes, "trades": trades, "grid": grid, "horizon": horizon, "grids": grids}, inputs={"quotes": quotes, "trades": trades})

    q = read_csv_ts(quotes).set_index("ts").sort_index()
    t = read_csv_ts(trades).set_index("ts").sort_index()

    q["mid"] = ms.midprice(q)
    q["quoted_spread"] = ms.quoted_spread(q)
    q["rel_quoted_spread"] = q["quoted_spread"] / q["mid"]

    mid_at_trade = q["mid"].reindex(t.index, method="ffill")
    mid_future = q["mid"].reindex(t.index + pd.Timedelta(horizon), method="ffill")

    sign = ms.tick_rule_sign(pd.to_numeric(t["price"], errors="coerce").dropna())
    t2 = t.loc[sign.index].copy()
    mid_at_trade2 = mid_at_trade.reindex(t2.index)
    mid_future2 = mid_future.reindex(t2.index)

    eff = ms.effective_spread(t2, mid_at_trade2)
    rs = ms.realized_spread(t2, mid_future2, sign)

    grid_list = [g.strip() for g in grids.split(",") if g.strip()]
    sig = ms.signature_plot(q["mid"], grid_list).sort_index()
    ...
    r_trd = fin.log_returns_from_close(pd.to_numeric(t1["price"], errors="coerce"))
    ...
    figs = [
        ...
        plots.plot_hist(q["quoted_spread"].dropna(), "Quoted spread", os.path.join(fig_dir, "quoted_spread_hist.png"), bins=80),
        plots.plot_hist(eff.dropna(), "Effective spread", os.path.join(fig_dir, "effective_spread_hist.png"), bins=80),
        plots.plot_hist(rs.dropna(), f"Realized spread (horizon={horizon})", os.path.join(fig_dir, "realized_spread_hist.png"), bins=80),
    ]

    def _grid_to_seconds(g: str) -> float:
        try:
            return float(pd.Timedelta(g).total_seconds())
        except Exception:
            return float("nan")

    x_sec = [_grid_to_seconds(g) for g in sig.index]
    figs.append(plots.plot_xy_line(x_sec, sig.to_numpy(), "Signature plot: realized variance vs sampling grid", os.path.join(fig_dir, "signature_plot.png"), xlabel="grid (seconds)", ylabel="realized variance"))

    summary = {
        "quotes": {
            "quoted_spread_mean": float(q["quoted_spread"].mean()),
            "quoted_spread_median": float(q["quoted_spread"].median()),
            "rel_quoted_spread_mean": float(q["rel_quoted_spread"].mean()),
        },
        "trades": {
            "effective_spread_mean": float(eff.mean()),
            "effective_spread_median": float(eff.median()),
            "realized_spread_mean": float(rs.mean()),
            "realized_spread_median": float(rs.median()),
            "horizon": horizon,
        },
        "signature_plot": {str(k): float(v) for k, v in sig.items()},
        "acf": {"grid": grid, "acf_mid": acf_mid, "acf_trade": acf_trd},
    }
    _write_json(out_dir, "artifacts/summary.json", summary)
    ...
    mc = model_card("Chapter 09", {
        ...
        "Realized spread": f"2*sign*(trade - mid(t+{horizon}))",
        "Signature plot": "RV(mid) vs sampling grid",
        "ACF": f"mid vs trade returns on grid={grid}",
    })
    ...
    return write_report(out_dir, "Chapter 09 — Spreads and microstructure noise (research-grade)", sections)

def chapter10(*, out_dir: str, trades: str, quotes: str | None = None) -> tuple[str, str]:
    """Roll spread estimator with optional comparison to quote-based spreads."""
    _prov(out_dir, "chapter10", params={"trades": trades, "quotes": quotes}, inputs={"trades": trades, "quotes": quotes})
    t = read_csv_ts(trades).set_index("ts").sort_index()
    roll = ms.roll_spread_from_trades(t["price"])

    fig_dir = ensure_dir(os.path.join(out_dir, "figures"))
    figs = []

    dp = pd.to_numeric(t["price"], errors="coerce").diff().dropna()
    figs.append(plots.plot_hist(dp, "Δ trade price", os.path.join(fig_dir, "dp_hist.png"), bins=120))
    ...
    if quotes:
        ...
        q["mid"] = ms.midprice(q)
        q["quoted_spread"] = ms.quoted_spread(q)
        comp["quoted_spread_mean"] = float(q["quoted_spread"].mean())
        comp["quoted_spread_median"] = float(q["quoted_spread"].median())

        mid_at_trade = q["mid"].reindex(t.index, method="ffill")
        eff = ms.effective_spread(t, mid_at_trade)
        comp["effective_spread_mean"] = float(eff.mean())
        comp["effective_spread_median"] = float(eff.median())
        figs.append(plots.plot_hist(q["quoted_spread"].dropna(), "Quoted spread", os.path.join(fig_dir, "quoted_spread_hist.png"), bins=80))
        figs.append(plots.plot_hist(eff.dropna(), "Effective spread", os.path.join(fig_dir, "effective_spread_hist.png"), bins=80))

    summary = {"roll": roll, "comparisons": comp}
    _write_json(out_dir, "artifacts/summary.json", summary)

    mc = model_card("Chapter 10", {
        "Estimator": "Roll (1984): spread ≈ 2*sqrt(-Cov(Δp_t, Δp_{t-1}))",
        "Inputs": "trade prices; optional quotes for comparison",
    })

    sections = [
        ("Model card", mc),
        ("Charts", _md_imgs(out_dir, figs) if figs else "(no figures)"),
        ("Roll estimate", "```json\n" + json.dumps(roll, indent=2) + "\n```"),
        ("Comparisons", "```json\n" + json.dumps(comp, indent=2) + "\n```" if comp else "No quotes provided."),
        ("Summary artifact", "Saved to `artifacts/summary.json`."),
    ]
    return write_report(out_dir, "Chapter 10 — Roll spread estimator (research-grade)", sections)
...
def chapter11(
    *,
    ...
    quotes: str,
    trades: str,
    sign_method: str = "tick",
    horizon: str = "10s",
    bins: int = 10,
) -> tuple[str, str]:
    """Adverse selection / impact proxy via Kyle lambda and response curves."""
    _prov(out_dir, "chapter11", params={"quotes": quotes, "trades": trades, "sign_method": sign_method, "horizon": horizon, "bins": bins}, inputs={"quotes": quotes, "trades": trades})
    q = read_csv_ts(quotes).set_index("ts").sort_index()
    t = read_csv_ts(trades).set_index("ts").sort_index()

    df = ms.kyle_lambda(t, q, horizon=horizon)
    if len(df) < 50:
        raise ValueError("Not enough aligned trade/quote observations for Kyle regression.")

    X = df[["x_signed_size"]]
    y = df["y_dmid"]
    fit = fit_ols(y, X, add_const=True, cov_type="HC1")
    summ = summarize(fit, names=["const", "lambda"], cov_type="HC1")

    tmp = df[["x_signed_size", "y_dmid"]].copy()
    tmp["bin"] = pd.qcut(tmp["x_signed_size"], q=min(bins, tmp["x_signed_size"].nunique()), duplicates="drop")
    resp = tmp.groupby("bin")["y_dmid"].mean()
    resp.index = resp.index.astype(str)

    fig_dir = ensure_dir(os.path.join(out_dir, "figures"))
    figs = [
        plots.plot_scatter_fit(tmp["x_signed_size"], tmp["y_dmid"], "Δmid vs signed size (with fit)", os.path.join(fig_dir, "kyle_scatter.png")),
        plots.plot_bar({k: float(v) for k, v in resp.items()}, "Mean Δmid by signed-size bin", os.path.join(fig_dir, "resp_by_bin.png")),
    ]

    summary = {
        "kyle_lambda_regression": summ.__dict__,
        "horizon": horizon,
        "bins": int(bins),
        "bucket_response": {str(k): float(v) for k, v in resp.items()},
    }
    _write_json(out_dir, "artifacts/summary.json", summary)

    mc = model_card("Chapter 11", {
        "Kyle model (proxy)": "Δmid ≈ λ * signed_volume + ε",
        "Signed volume": "tick-rule sign * trade size (toy)",
        "Horizon": horizon,
        "OLS": "HC1 robust SE (via cov_type)",
        "Diagnostics": "bucketed response curve",
    })

    sections = [
        ("Model card", mc),
        ("Charts", _md_imgs(out_dir, figs)),
        ("Regression summary", summary_md(summ)),
        ("Summary artifact", "Saved to `artifacts/summary.json`."),
    ]
    return write_report(out_dir, "Chapter 11 — Impact proxy and adverse selection (research-grade)", sections)
...
def chapter12(
    *,
    ...
    """LOB queueing baseline: birth–death rates and depletion-probability calibration."""
    _prov(out_dir, "chapter12", params={"lob": lob, "bins": bins, "horizon_s": horizon_s, "sims": sims, "sample_n": sample_n}, inputs={"lob": lob})
    ...
    imb = (df["bid_size"] - df["ask_size"]) / (df["bid_size"] + df["ask_size"])
    dmid = df["mid"].diff().shift(-1)
    y_up = (dmid > 0).astype(int)
    base = pd.DataFrame({"imb": imb, "y_up": y_up}).dropna()
    base["bin"] = pd.qcut(base["imb"], q=min(bins, base["imb"].nunique()), duplicates="drop")
    p_up_by_bin = base.groupby("bin")["y_up"].mean()
    p_up_by_bin.index = p_up_by_bin.index.astype(str)

    rates = ms.queue_birth_death_rates(df.reset_index())
    lam_b = rates.get("bid_lambda_arrival_per_s", float("nan"))
    mu_b = rates.get("bid_mu_departure_per_s", float("nan"))
    lam_a = rates.get("ask_lambda_arrival_per_s", float("nan"))
    mu_a = rates.get("ask_mu_departure_per_s", float("nan"))

    sample = df.dropna(subset=["bid_size", "ask_size", "mid"]).copy()
    if len(sample) > int(sample_n):
        sample = sample.iloc[np.linspace(0, len(sample) - 2, int(sample_n)).astype(int)]
    sample["dmid_next"] = df["mid"].diff().shift(-1).reindex(sample.index)
    sample["y_up"] = (sample["dmid_next"] > 0).astype(int)

    # Cache depletion probabilities by rounded integer queue size
    ask_q = sample["ask_size"].round().astype(int).clip(lower=0)
    bid_q = sample["bid_size"].round().astype(int).clip(lower=0)

    ask_cache = {q0: ms.bd_depletion_prob(float(q0), lam_a, mu_a, float(horizon_s), sims=int(sims), rng_seed=123) for q0 in ask_q.unique()}
    bid_cache = {q0: ms.bd_depletion_prob(float(q0), lam_b, mu_b, float(horizon_s), sims=int(sims), rng_seed=456) for q0 in bid_q.unique()}

    sample["p_deplete_ask"] = ask_q.map(ask_cache)
    sample["p_deplete_bid"] = bid_q.map(bid_cache)
    sample["p_up_pred"] = sample["p_deplete_ask"] / (sample["p_deplete_ask"] + sample["p_deplete_bid"] + 1e-12)

    cal = sample[["p_up_pred", "y_up"]].dropna()
    cal["bin"] = pd.qcut(cal["p_up_pred"], q=min(bins, cal["p_up_pred"].nunique()), duplicates="drop")
    cal_emp = cal.groupby("bin")["y_up"].mean()
    cal_emp.index = cal_emp.index.astype(str)

    fig_dir = ensure_dir(os.path.join(out_dir, "figures"))
    figs = [
        plots.plot_series(imb.dropna(), "Imbalance (best level)", os.path.join(fig_dir, "imb_series.png")),
        plots.plot_bar({str(k): float(v) for k, v in p_up_by_bin.items()}, "P(up) by imbalance bin", os.path.join(fig_dir, "pup_by_imb_bin.png")),
        plots.plot_bar({str(k): float(v) for k, v in cal_emp.items()}, f"Calibration: empirical P(up) by predicted p_up bin (H={horizon_s}s)", os.path.join(fig_dir, "calibration_pup.png")),
    ]

    summary = {
        "rates": rates,
        "imbalance_baseline": {str(k): float(v) for k, v in p_up_by_bin.items()},
        "depletion_model": {
            "horizon_s": float(horizon_s),
            "sims": int(sims),
            "sample_n": int(len(sample)),
            "calibration": {str(k): float(v) for k, v in cal_emp.items()},
            "pred_pup_mean": float(sample["p_up_pred"].mean()),
        },
    }
    _write_json(out_dir, "artifacts/summary.json", summary)

    mc = model_card("Chapter 12", {
        "Imbalance": "imb = (bid_size-ask_size)/(bid_size+ask_size)",
        "Queue model": "Independent birth–death queues (bid/ask), constant rates",
        "Rates": "estimated from snapshot deltas (toy)",
        "Prediction": "p_up ≈ p(deplete ask) / (pA + pB)",
        "Calibration": f"qcut into {bins} bins; horizon={horizon_s}s",
        "Speed": "Monte Carlo + caching by integer queue size; increase sims/sample_n for higher fidelity",
    }, notes=[
        "This is a baseline queueing proxy; real TAQ/LOB feeds need event-level data.",
    ])
    ...
    return write_report(out_dir, "Chapter 12 — LOB queueing baseline and calibration (research-grade)", sections)
