...
def build(book_path: str, out_dir: str, data_root: str | None = None) -> tuple[str, str]:
    ...
    for item in chapters:
        ...
        # resolve any file paths relative to data_root
        for k in ["prices", "prices_multi", "returns", "trades", "quotes", "events", "lob"]:
            if k in args and isinstance(args[k], str):
                args[k] = _resolve_path(args[k], data_root)

        fn = CHAPTERS[key]
        if key == "chapter02":
            md_path, html_path = fn(args["prices"], chapter_out, lags=int(args.get("lags", 20)))
        elif key == "chapter03":
            md_path, html_path = fn(args["prices"], chapter_out, lags=int(args.get("lags", 20)), vr_q=int(args.get("vr_q", 5)))
        elif key == "chapter04":
            md_path, html_path = fn(args["returns"], chapter_out)
        elif key == "chapter05":
            md_path, html_path = fn(args["prices_multi"], chapter_out, J=int(args.get("J", 60)), K=int(args.get("K", 20)), top_frac=float(args.get("top_frac", 0.25)))
        elif key == "chapter06":
            md_path, html_path = fn(args["prices_multi"], chapter_out, pair=str(args.get("pair", "A,B")), z_win=int(args.get("z_win", 60)), z_entry=float(args.get("z_entry", 1.5)))
        elif key == "chapter07":
            md_path, html_path = fn(args["prices_multi"], chapter_out)
        elif key == "chapter08":
            md_path, html_path = fn(args["events"], chapter_out)
        elif key == "chapter09":
            md_path, html_path = fn(args["quotes"], args["trades"], chapter_out)
        elif key == "chapter10":
            md_path, html_path = fn(args["trades"], chapter_out)
        elif key == "chapter11":
            md_path, html_path = fn(args["quotes"], args["trades"], chapter_out)
        elif key == "chapter12":
            md_path, html_path = fn(args["lob"], chapter_out)
        elif key == "chapter13":
            md_path, html_path = fn(chapter_out, **{k:v for k,v in args.items() if k not in ["prices","prices_multi","returns","trades","quotes","events","lob"]})
        elif key == "chapter14":
            md_path, html_path = fn(chapter_out)
        else:
