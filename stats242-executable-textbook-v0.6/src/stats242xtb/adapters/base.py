...
def _find_col(df: pd.DataFrame, candidates: list[str]) -> Optional[str]:
    ...
    # allow regex candidates like r"^TRDPRC\d+$"
    for cand in candidates:
...
def parse_timestamp(df: pd.DataFrame, mapping: Dict[str, Any] | None = None) -> pd.Series:
    """Return a UTC pandas datetime series from many common patterns.

    Supported:
    - existing `ts` or `timestamp`
    - separate `date` + `time` columns
    - epoch seconds/ms/us/ns via `epoch` columns
    Mappings can override via:
      mapping:
        ts: "colname"  OR
        date: "DATE" , time: "TIME"  OR
        epoch: "TIMESTAMP_NANOS", epoch_unit: "ns"
    """
    m = mapping or {}
    if "ts" in m:
        col = m["ts"]
    ...
    if date_col is not None and time_col is not None:
        # join as string, let pandas parse
...
def canonicalize_trades(df: pd.DataFrame, schema: str = "auto", mapping: Dict[str, Any] | None = None) -> pd.DataFrame:
    ...
    if "price" in m:
        ...
        # TAQ-ish: PRICE, TRDPRC1, TradePrice
        pcol = _find_col(df, ["price", "PRICE", "TradePrice", "re:^TRDPRC\d+$"])
    if pcol is None:
...
def canonicalize_quotes(df: pd.DataFrame, schema: str = "auto", mapping: Dict[str, Any] | None = None) -> pd.DataFrame:
    ...
    if bid_sz_col is not None:
        out["bid_size"] = pd.to_numeric(df[bid_sz_col], errors="coerce")
    if ask_sz_col is not None:
        out["ask_size"] = pd.to_numeric(df[ask_sz_col], errors="coerce")
