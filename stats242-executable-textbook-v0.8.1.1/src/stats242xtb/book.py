...
def build(book_path: str, out_dir: str, data_root: str | None = None) -> tuple[str, str]:
    ...
    for item in chapters:
        ...
        fn = CHAPTERS[key]
        md_path, html_path = fn(out_dir=chapter_out, **args)
...
"""
    ...
    with open(manifest_path, "w", encoding="utf-8") as f:
        ...
        # Book-level provenance (best-effort)
    try:
        from .provenance import collect_env, maybe_git_hash
        build_meta_path = os.path.join(book_dir, "build_meta.json")
        bm = {
            "timestamp_utc": datetime.utcnow().isoformat() + "Z",
            "book": os.path.basename(book_path),
            "git": {"hash": maybe_git_hash(os.path.abspath(os.path.join(base, '..')))},
            "env": collect_env(),
        }
